...
def evaluate_model(data_folder, output_folder):
    ...
    # ** you have to remove this part, this is made by pavan**
    # Compute confusion matrix
    from sklearn.metrics import confusion_matrix
    cm = confusion_matrix(labels, binary_outputs)
    tn, fp, fn, tp = cm.ravel()  # Extract TN, FP, FN, TP

    # Compute True Positive Rate (Sensitivity/Recall)
    tpr = tp / (tp + fn) if (tp + fn) > 0 else 0

    # Print confusion matrix and TPR
    print("\nConfusion Matrix:")
    print(cm)
    print(f"\nTrue Positive Rate (Sensitivity): {tpr:.3f}")
    # **remove till here also remove last 2 metrics in return below**
